...
public class HBaseConfiguration extends Configuration
{
	...
	{
		// replace with private constructor, HBaseConfiguration should not
	...
	{
		// replace with private constructor
		this();
		merge(this, c);
	}
	...
	private static void checkDefaultsVersion(Configuration conf)
	{
		...
		{
			throw new RuntimeException("hbase-default.xml file seems to be for and old version of HBase (" + defaultsVersion
	...
	public static Configuration addHbaseResources(Configuration conf, String hbaseSiteFilePath)
	{
		conf.addResource("hbase-default.xml");
		...
		try
		{
			fis = new FileInputStream(hbaseSiteFilePath);
		...
		{
			throw new RuntimeException("无法读取到Hbase配置文件信息：" + hbaseSiteFilePath);
		}
		catch (Exception e)
		{
			throw new RuntimeException("无法读取到Hbase配置文件信息：" + e.toString());
		}
		finally
		{
			if (null != fis)
	...
	public static Configuration create(String hbaseSiteFilePath)
	{
		Configuration conf = new Configuration();
		conf.setClassLoader(HBaseConfiguration.class.getClassLoader());
		return addHbaseResources(conf, hbaseSiteFilePath);
	}
	...
	{
		Configuration conf = new Configuration();
		// In case HBaseConfiguration is loaded from a different classloader
		// than
	...
	public static boolean isShowConfInServlet()
	{
		...
		{
			// should we handle it more aggressively in addition to log the
			// error?
	 ...
	 * possibly referring to the deprecated name of the configuration property.
	 ...
	 * @param deprecatedName a deprecatedName for the property to use if
	 *            non-deprecated name is not used
	 * @param defaultValue default value.
	 * @throws NumberFormatException when the value is invalid
	 * @return property value as an <code>int</code>, or
	 *         <code>defaultValue</code>.
	 */
	// developer note: This duplicates the functionality of deprecated
	// property support in Configuration in Hadoop 2. But since Hadoop-1 does
	// not
	...
	/**
	 * Get the password from the Configuration instance using the getPassword
	 * method if it exists. If not, then fall back to the general get method for
	...
	/**
	 * For debugging. Dump configurations to system output as xml format. Master
	 * and RS configurations can also be dumped using http services. e.g.
	 * "curl http://master:16010/dump"
	 */
