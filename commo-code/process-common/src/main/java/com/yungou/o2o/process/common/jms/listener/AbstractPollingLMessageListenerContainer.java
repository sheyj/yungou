...
/**
 * 重写org.springframework.jms.listener.AbstractPollingMessageListenerContainer
 * 
 * @author lin.zb
 * @date 2016年1月19日 下午5:51:15
 * @version 1.0.0 
 * @copyright wonhigh.net.cn
 */
public abstract class AbstractPollingLMessageListenerContainer extends AbstractLMessageListenerContainer {
	private static final Logger LOG = LoggerFactory.getLogger(AbstractPollingLMessageListenerContainer.class);
	...
	protected boolean doReceiveAndExecute(Object invoker, Session session, MessageConsumer consumer,
		...
		try {
			...
			if (sessionToUse == null) {
				sessionToUse = ConnectionFactoryUtils.doGetTransactionalSession(getConnectionFactory(),
			...
			if (message != null) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("Received message of type [" + message.getClass() + "] from consumer [" + consumerToUse
				...
				try {
					...
					if (status != null) {
						if (LOG.isDebugEnabled()) {
							LOG.debug("Rolling back transaction because of listener exception thrown: " + ex);
						}
				...
				if (LOG.isTraceEnabled()) {
					LOG.trace("Consumer [" + consumerToUse + "] of " + (transactional ? "transactional " : "")
	...
	protected boolean isSessionLocallyTransacted(Session session) {
		...
		return (resourceHolder == null || resourceHolder instanceof LocallyExposedJmsResourceHolder || !resourceHolder
	...
	private void rollbackOnException(TransactionStatus status, Throwable ex) {
		...
		try {
			...
			LOG.error("Listener exception overridden by rollback exception", ex);
			throw ex2;
		}
		catch (Error err) {
			LOG.error("Listener exception overridden by rollback error", ex);
			throw err;
		}
	}
	...
	private class MessageListenerContainerResourceFactory implements ConnectionFactoryUtils.ResourceFactory {
		...
		public Connection getConnection(JmsResourceHolder holder) {
			return AbstractPollingLMessageListenerContainer.this.getConnection(holder);
		}

		public Session getSession(JmsResourceHolder holder) {
			return AbstractPollingLMessageListenerContainer.this.getSession(holder);
		}

		public Connection createConnection() throws JMSException {
			if (AbstractPollingLMessageListenerContainer.this.sharedConnectionEnabled()) {
				Connection sharedCon = AbstractPollingLMessageListenerContainer.this.getSharedConnection();
				return new SingleConnectionFactory(sharedCon).createConnection();
			}
			else {
				return AbstractPollingLMessageListenerContainer.this.createConnection();
			}
		}

		public Session createSession(Connection con) throws JMSException {
			return AbstractPollingLMessageListenerContainer.this.createSession(con);
		}

		public boolean isSynchedLocalTransactionAllowed() {
			return AbstractPollingLMessageListenerContainer.this.isSessionTransacted();
		}
	}
}
