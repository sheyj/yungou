...
/**
 * 重写org.springframework.jms.listener.DefaultMessageListenerContainer
 * 
 * @author lin.zb
 * @date 2016年1月19日 下午5:49:23
 * @version 1.0.0 
 * @copyright wonhigh.net.cn
 */
public class DefaultLMessageListenerContainer extends AbstractPollingLMessageListenerContainer {
	private static final Logger LOG = LoggerFactory.getLogger(DefaultLMessageListenerContainer.class);

	/**
	 * Default thread name prefix: "DefaultLMessageListenerContainer-".
	 */
	public static final String DEFAULT_THREAD_NAME_PREFIX = ClassUtils
			.getShortName(DefaultLMessageListenerContainer.class) + "-";
	...
	private static final Constants constants = new Constants(DefaultLMessageListenerContainer.class);
	...
	protected void doShutdown() throws JMSException {
		LOG.debug("Waiting for shutdown of message listener invokers");
		try {
			synchronized (this.lifecycleMonitor) {
				// Waiting for AsyncMessageListenerInvokers to deactivate themselves...
				while (this.activeInvokerCount > 0) {
					if (LOG.isDebugEnabled()) {
						LOG.debug("Still waiting for shutdown of " + this.activeInvokerCount
		...
		if (isRunning()) {
			...
			synchronized (this.lifecycleMonitor) {
				if (this.scheduledInvokers.size() < this.maxConcurrentConsumers
					...
					scheduleNewInvoker();
					if (LOG.isDebugEnabled()) {
						LOG.debug("Raised scheduled invoker count: " + this.scheduledInvokers.size());
					}
				}
			}
		}
	...
	protected void establishSharedConnection() {
		try {
			...
			LOG.debug("Could not establish shared JMS Connection - "
	...
	protected void startSharedConnection() {
		try {
			...
			LOG.debug("Connection start failed - relying on listeners to perform recovery", ex);
		}
	}
	...
	protected void stopSharedConnection() {
		try {
			...
			LOG.debug("Connection stop failed - relying on listeners to perform recovery after restart", ex);
		}
	}
	...
	protected void handleListenerSetupFailure(Throwable ex, boolean alreadyRecovered) {
		...
		if (ex instanceof SharedConnectionNotInitializedException) {
			if (!alreadyRecovered) {
				LOG.info("JMS message listener invoker needs to establish shared Connection");
			}
		}
		else {
			// Recovery during active operation..
			if (alreadyRecovered) {
				LOG.debug("Setup of JMS message listener invoker failed - already recovered by other invoker", ex);
			}
			else {
	...
	protected void refreshConnectionUntilSuccessful() {
		while (isRunning()) {
			try {
				...
				LOG.info("Successfully refreshed JMS Connection");
				break;
			}
			catch (Exception ex) {
				if (ex instanceof JMSException) {
			...
			try {
				...
				synchronized (lifecycleMonitor) {
					if (!shouldRescheduleInvoker(this.idleTaskExecutionCount) || !rescheduleTaskIfNecessary(this)) {
						...
						if (LOG.isDebugEnabled()) {
							LOG.debug("Lowered scheduled invoker count: " + scheduledInvokers.size());
						}
						lifecycleMonitor.notifyAll();
						clearResources();
					}
						...
						if (nonPausedConsumers < 1) {
							LOG.error("All scheduled consumers have been paused, probably due to tasks having been rejected. "
									+ "Check your thread pool configuration! Manual recovery necessary through a start() call.");
						}
						else if (nonPausedConsumers < getConcurrentConsumers()) {
							LOG.warn("Number of scheduled consumers has dropped below concurrentConsumers limit, probably "
